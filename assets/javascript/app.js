...
function addToFavorites(zID) {
    ...
    if (favorites.indexOf(zID) === -1) {
        ...
        document.cookie = "giforjif=; expires=Thu, 01 Jan 1970 00:00:00 UTC";

        // Store the username as a cookie using "document.cookie"
        document.cookie = "giforjif=" + favoriteString + ";";
...
//Look for the cookie with favorites.. if it exists, build the 'favoriteString' and render the favorites. Else do nothing.
function readCookie(name) {
    ...
    for (var i = 0; i < ca.length; i++) {
      ...
      if (c.indexOf(nameEQ) === 0) {
        ...
        favoriteString = c.substring(nameEQ.length, c.length);
        favorites =   c.substring(nameEQ.length, c.length).split(",");
        displayFavoriteGifs();
        return;
      }
    }
